...
export default function Menu() {
    ...
    useEffect(() => {
        const user = JSON.parse(localStorage.getItem('user'));  // Recuperamos los datos del usuario

        // Verificamos si el usuario existe en localStorage
        if (user) {
            console.log('ESTA LOGUEADO:', user);  // Mostrar mensaje si el usuario está logueado
        } else {
            console.log('No hay usuario logueado');  // Mostrar mensaje si no hay usuario logueado
        }
            ...
            <section className={s.heading}>
                ...
                <div className={s.categories}>
                    {categories.map(({ name, icon }, catKey) => (
                        <span key={catKey} className={s.category} onClick={() => gotoArticleCategory("." + name)}>
